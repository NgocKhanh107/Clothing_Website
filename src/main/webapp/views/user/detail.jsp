						...
						<c:forEach var="item" items="${loadMultiImage}">
							<div class="carousel-item">
								<img class="w-100 h-100" height="570px"
									src="/upload/${item.image}" alt="Image">
							</div>
						</c:forEach>
						<!-- <div class="carousel-item">
				...
				<div class="h-100 bg-light p-30">
					...
					<form action="/shop/detail/${sessionScope.cateId}"
						...
						<div class="d-flex mb-3">
							...
							<c:forEach var="item" items="${loadMultiSize}">
								...
								<div class="form-check form-check-inline">
									...
									<label class="form-check-label" >${item.size.name}</label>
								</div>
							</c:forEach>
						...
						<div class="d-flex mb-4">
							...
							<c:forEach var="item" items="${loadMultiColor}">

								<div class="form-check form-check-inline">
									<input type="radio" name="color" class="form-check-input"
										value="${item.color.id}"
										${checkColor==item.color.id?'checked':''} /> <label
										class="form-check-label" >${item.color.name}</label>
								</div>
